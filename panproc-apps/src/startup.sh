...
#################################
# condaenv - activate conda env
flag="--condaenv"
default=""
CONDAENVPASSED="False"
opts_CheckFlagBasic $flag $@
FlagExists=`echo $?`
if [ $FlagExists -eq 1 ]
then
  ...
  then
      ...
      if [ $FlagExists -eq 1 ]
      then 
         CONDAENV=`opts_GetOpt1 $flag $@`
         CONDAENVPASSED="True" 
         MSG="--condaenv parsed. The following conda environments are available:"
         if [ $LOG -eq 1 ]
         then 
           echo -e $MSG
         ...
         MSG="--condaenv needs an included path"
         if [ $LOG -eq 1 ]; then echo -e $MSG; fi
      fi
   fi
#else
...
# --sourcepre
if [ $CONDAENVPASSED = "True" ]
then
   MSG="Activating $CONDAENV"
   if [ $LOG -eq 1 ]; then echo -e $MSG; fi
    # this is not working try more direct approach
    #export PYTHONPATH='/opt/miniconda/envs/ciftify/lib/python36.zip:/opt/miniconda/envs/ciftify/lib/python3.6:/opt/miniconda/envs/ciftify/lib/python3.6/lib-dynload:/opt/miniconda/envs/ciftify/lib/python3.6/site-packages:${PYTHONPATH}'
    #export PATH=/opt/miniconda/envs/ciftify/bin:${PATH}
    #export LD_LIBRARY_PATH=/opt/miniconda/envs/ciftify/lib:${LD_LIBRARY_PATH}
    . /panbin/activateConda.sh $CONDAENV
